...
const DEAL_STAGE_SET = new Set<DealStage>(DEAL_STAGE_ORDER);

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === "object" && value !== null;
}

function normalizeDealListItem(value: unknown): DealListItem | null {
  if (!isRecord(value)) return null;

  const stage = typeof value.stage === "string" && DEAL_STAGE_SET.has(value.stage as DealStage)
    ? (value.stage as DealStage)
    : "LEAD";
  const company = isRecord(value.company) && typeof value.company.id === "string" && typeof value.company.name === "string"
    ? { id: value.company.id, name: value.company.name }
    : null;
  const owner = isRecord(value.owner) && typeof value.owner.id === "string" && typeof value.owner.email === "string"
    ? {
        id: value.owner.id,
        email: value.owner.email,
        name: typeof value.owner.name === "string" ? value.owner.name : null,
        image: typeof value.owner.image === "string" ? value.owner.image : null,
      }
    : null;

  return {
    id: typeof value.id === "string" ? value.id : "",
    name: typeof value.name === "string" && value.name.trim().length > 0 ? value.name : "Untitled deal",
    stage,
    amount: typeof value.amount === "number" && Number.isFinite(value.amount) ? value.amount : 0,
    source: typeof value.source === "string" ? value.source as DealListItem["source"] : "OTHER",
    expectedCloseDate: typeof value.expectedCloseDate === "string" ? value.expectedCloseDate : null,
    ...
    contacts: Array.isArray(value.contacts)
      ? value.contacts.filter(isRecord).map((contact) => ({
          ...
          companyId: typeof contact.companyId === "string" ? contact.companyId : null,
          company:
            isRecord(contact.company) &&
            typeof contact.company.id === "string" &&
            typeof contact.company.name === "string"
    ...
    _count: isRecord(value._count)
      ? {
          meetings:
            typeof value._count.meetings === "number" && Number.isFinite(value._count.meetings)
              ? value._count.meetings
              : 0,
          contacts:
            typeof value._count.contacts === "number" && Number.isFinite(value._count.contacts)
              ? value._count.contacts
              : 0,
        }
      : { meetings: 0, contacts: 0 },
    lastMeetingAt: typeof value.lastMeetingAt === "string" ? value.lastMeetingAt : null,
    createdAt: typeof value.createdAt === "string" ? value.createdAt : new Date(0).toISOString(),
    updatedAt: typeof value.updatedAt === "string" ? value.updatedAt : new Date(0).toISOString(),
  };
}

function normalizeDealList(payload: unknown): DealListItem[] {
  if (!Array.isArray(payload)) return EMPTY_DEALS;
  return payload.map(normalizeDealListItem).filter((deal): deal is DealListItem => Boolean(deal?.id));
}
...
export function DealsDashboard() {
  ...
  const resource = useDashboardResource<DealListItem[]>({
    ...
    load: async ({ signal, refresh }) => {
      ...
      return normalizeDealList(payload);
    },
  });
