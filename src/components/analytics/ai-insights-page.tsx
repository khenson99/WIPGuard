...
import { AlertCircle, AlertTriangle, BarChart3, Info } from "lucide-react";
...
export function AiInsightsPage() {
  ...
  }, [allInsights, isPinned, sectionFilter, severityFilter, showDismissed, sortAndFilter, sortMode]);
  ...
  const infoCount = allInsights.filter((insight) => insight.severity === "info").length;
  const avgConfidence =
  ...
  return (
    <div className="space-y-6">
      ...
      {error ? (
        <DashboardErrorBanner message={error} onRetry={resource.refresh} retryLabel="Rerun insights" />
      ) : null}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex gap-1 rounded-lg bg-secondary/50 p-0.5" role="group" aria-label="Section filter">
          {(["all", "ads-traffic", "finance", "sales-pipeline", "customer-success"] as SectionFilter[]).map((section) => (
            <button
              key={section}
              type="button"
              onClick={() => {
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-border bg-card p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              Showing <span className="font-medium text-foreground tabular-nums">{pageStartIndex + 1}</span>-
