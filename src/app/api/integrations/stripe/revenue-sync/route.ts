...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const permission = await enforcePermission({
      ...
      targetId: IntegrationProvider.STRIPE,
    });
    ...
    console.error("GET /api/integrations/stripe/revenue-sync error:", error);
    return NextResponse.json(
      { error: "Failed to load Stripe revenue sync rule" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    console.error("POST /api/integrations/stripe/revenue-sync error:", error);
    const message = error instanceof Error ? error.message : "Failed to run Stripe revenue sync";
