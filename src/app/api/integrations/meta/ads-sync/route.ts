...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const permission = await enforcePermission({
      ...
      targetId: IntegrationProvider.META_ADS,
    });
    ...
    console.error("GET /api/integrations/meta/ads-sync error:", error);
    return NextResponse.json(
      { error: "Failed to load Meta ads sync rule" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    console.error("POST /api/integrations/meta/ads-sync error:", error);
    const message = error instanceof Error ? error.message : "Failed to run Meta ads sync";
