...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const rule = await getOrCreateProviderMetricsRule({
      userId: ownerUserId,
      ruleKey: PYLON_CONVERSATION_SYNC_RULE_KEY,
    });
    ...
    console.error("GET /api/integrations/pylon/conversation-sync error:", error);
    return NextResponse.json(
      { error: "Failed to load Pylon conversation sync rule" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    console.error("POST /api/integrations/pylon/conversation-sync error:", error);
    const message = error instanceof Error ? error.message : "Failed to run Pylon conversation sync";
