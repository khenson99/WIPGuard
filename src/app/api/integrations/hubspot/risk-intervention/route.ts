...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const rule = await getOrCreateHubSpotRiskRule(ownerUserId);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (action === "configure") {
      const rule = await patchHubSpotRiskRule(ownerUserId, {
    ...
    const result = await withSyncObservability(
      "hubspot", "risk-intervention", ownerUserId,
      () => runHubSpotRiskIntervention({ userId: ownerUserId, dryRun: body.dryRun }),
      { dryRun: body.dryRun },
    );
