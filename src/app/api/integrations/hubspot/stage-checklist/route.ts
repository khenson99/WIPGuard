...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const rule = await getOrCreateHubSpotChecklistRule(ownerUserId);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (action === "configure") {
      const rule = await patchHubSpotRule(ownerUserId, {
    ...
    const result = await withSyncObservability(
      "hubspot", "stage-checklist", ownerUserId,
      () => runHubSpotStageChecklist({ userId: ownerUserId, dryRun: body.dryRun }),
      { dryRun: body.dryRun },
    );
