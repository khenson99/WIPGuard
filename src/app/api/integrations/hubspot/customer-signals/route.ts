...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const rule = await getOrCreateHubSpotCustomerSignalRule(ownerUserId);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (action === "configure") {
      const rule = await patchHubSpotCustomerSignalRule(ownerUserId, {
    ...
    const result = await withSyncObservability(
      "hubspot", "customer-signals", ownerUserId,
      () => runHubSpotCustomerSignalAutomation({ userId: ownerUserId, dryRun: body.dryRun }),
      { dryRun: body.dryRun },
    );
