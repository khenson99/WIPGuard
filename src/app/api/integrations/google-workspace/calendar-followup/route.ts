...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const rule = await getOrCreateGoogleCalendarRule(ownerUserId);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (action === "configure") {
      const rule = await patchGoogleCalendarRule(ownerUserId, {
    ...
    const result = await withSyncObservability(
      "google_calendar", "calendar-followup", ownerUserId,
      () => runGoogleCalendarPrepFollowup({ userId: ownerUserId, dryRun: body.dryRun }),
      { dryRun: body.dryRun },
    );
