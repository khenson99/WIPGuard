...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    console.error("GET /api/integrations/linear/issue-sync error:", error);
    return NextResponse.json(
      { error: "Failed to load Linear sync rule" },
      { status: 500 },
    );
  }
}
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    console.error("POST /api/integrations/linear/issue-sync error:", error);
    const message = error instanceof Error ? error.message : "Failed to run Linear sync";
