...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const permission = await enforcePermission({
      ...
      targetId: IntegrationProvider.MERCURY,
    });
    ...
    console.error("GET /api/integrations/mercury/cashflow-sync error:", error);
    return NextResponse.json(
      { error: "Failed to load Mercury cashflow sync rule" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    console.error("POST /api/integrations/mercury/cashflow-sync error:", error);
    const message = error instanceof Error ? error.message : "Failed to run Mercury cashflow sync";
