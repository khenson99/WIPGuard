...
function parseTokenResponse(raw: unknown): {
  ...
  if (!accessToken) {
    throw new HubSpotCustomerSignalAuthError("HubSpot token refresh response missing access token");
  }
...
async function refreshHubSpotAccessToken(connection: IntegrationConnection): Promise<string> {
  ...
  if (!refreshToken) {
    throw new HubSpotCustomerSignalAuthError("HubSpot refresh token is missing");
  }

  if (!process.env.HUBSPOT_CLIENT_ID || !process.env.HUBSPOT_CLIENT_SECRET) {
    throw new HubSpotCustomerSignalAuthError("HubSpot OAuth client credentials are missing");
  }
  ...
  if (!response.ok) {
    ...
    throw new HubSpotCustomerSignalAuthError(reason);
  }
...
async function getValidHubSpotAccessToken(
  ...
  if (!token) {
    throw new HubSpotCustomerSignalAuthError("HubSpot access token is missing");
  }
