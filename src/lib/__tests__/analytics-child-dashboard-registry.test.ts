...
const OPS_DOMAINS = new Set(["decisionDashboard", "flowMetrics", "flowRisk", "observability"]);

describe("integration child dashboard registry", () => {
  it("registers a detailed dashboard component for every non-ops child route", () => {
    const nonOpsChildren = ANALYTICS_SUB_SECTIONS.filter((section) => !OPS_DOMAINS.has(section.dataDomain));
