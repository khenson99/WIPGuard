...
describe("PoolMonitor", () => {
  ...
  it("should recover from critical once exhaustion events age out", () => {
    vi.spyOn(console, "warn").mockImplementation(() => {});
    vi.useFakeTimers();
    try {
      PoolMonitorModule.poolMonitor.attach(mockPool as unknown as Pool, 25);
  ...
  it("should track transient connect retries", () => {
    PoolMonitorModule.poolMonitor.attach(mockPool as unknown as Pool, 25);

    PoolMonitorModule.poolMonitor.recordConnectRetry(
      new Error("timeout exceeded when trying to connect")
    );

    const metrics = PoolMonitorModule.poolMonitor.getMetrics();
    expect(metrics.totalConnectRetries).toBe(1);
    expect(metrics.lastError).toBe("timeout exceeded when trying to connect");
    expect(metrics.lastErrorAt).toBeTruthy();
  });
