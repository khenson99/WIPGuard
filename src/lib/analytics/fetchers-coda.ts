...
export async function fetchCodaData(
  ...
  // Step 3: Get columns to build a lookup from column name (lowercased) to column ID
  ...
  // Step 5: Map rows to CodaCard objects using column ID lookups
  const cards: CodaCard[] = rows.map((row) => {
    const vals = row.values || {};

    const name = nameColumnId
      ? (String(vals[nameColumnId] ?? "") || row.name || `Card ${row.id}`)
      : (row.name || `Card ${row.id}`);

    const status = statusColumnId
      ? (String(vals[statusColumnId] ?? "") || "Backlog")
      : "Backlog";

    const priority = priorityColumnId
      ? (vals[priorityColumnId] != null ? String(vals[priorityColumnId]) : undefined)
      : undefined;

    const assignee = assigneeColumnId
      ? (vals[assigneeColumnId] != null ? String(vals[assigneeColumnId]) : undefined)
      : undefined;
