...
export async function getCredentials(userId?: string): Promise<AnalyticsCredentials> {
  let hubspotToken: string | null = process.env.HUBSPOT_ACCESS_TOKEN || null;
  const stripeKey: string | null = process.env.STRIPE_SECRET_KEY || null;
  const mercuryKey: string | null = process.env.MERCURY_API_TOKEN || null;
