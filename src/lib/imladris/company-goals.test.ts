...
describe("buildCompanyGoalsDashboard", () => {
  ...
  it("loads legacy plural Linear project object types", async () => {
    const prisma = prismaMock([
      {
        ...projectRecord({
          id: "plural_project",
          name: "Plural object type project",
          state: "started",
          issues: [
            {
              id: "issue_plural_type",
              completedAt: null,
              archivedAt: null,
              state: { type: "started", name: "In Progress" },
            },
    ...
    expect(prisma.imladrisRawSourceRecord.findMany).toHaveBeenCalledWith(expect.objectContaining({
      where: expect.objectContaining({
        objectType: { in: expect.arrayContaining(["projects", "Projects"]) },
      }),
    }));
    expect(dashboard.goals).toEqual([
      expect.objectContaining({
