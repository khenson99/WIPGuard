...
describe("Imladris raw ingestion", () => {
  ...
  it("singularizes plural raw object types before deduping batch inputs", async () => {
    const { prisma } = createPrismaMock();
      ...
      records: [
        {
          ...
          externalId: "sub_plural_duplicate",
          sourceUpdatedAt: "2026-05-31T10:00:00.000Z",
          payload: {
            id: "sub_plural_duplicate",
  ...
  it("preserves singular raw object types that already end in sis", async () => {
    const { prisma } = createPrismaMock();
  ...
  it("normalizes plural analyses raw object types to analysis", async () => {
    const { prisma } = createPrismaMock();
  ...
  it("normalizes plural indices raw object types to index", async () => {
    const { prisma } = createPrismaMock();
    ...
    const result = await ingestImladrisRawRecords({
      ...
      records: [
        {
          objectType: "indices",
          externalId: "search_index_primary",
          payload: {
  ...
  it("preserves singular raw object types that already end in ries", async () => {
    const { prisma } = createPrismaMock();
  ...
  it("preserves analytics raw object types", async () => {
    const { prisma } = createPrismaMock();
      ...
      records: [
        {
          ...
          externalId: "analytics_activation",
          payload: {
